...
#!/bin/bash
# WARP 流媒体解锁检测脚本（所有请求强制通过 WARP IPv6 接口发出）
...
# 获取当前 WARP IPv6 出口地址
get_ipv6() { curl -6 --interface $IFACE -s --max-time 5 https://ip.gs || echo "不可用"; }
...
# Netflix 检测（强制走 WARP IPv6 接口）
check_netflix() {
    local sg_id="81215567"       # 非自制剧 ID
    local original_id="80018499" # 自制剧 ID
    local code_sg
    code_sg=$(curl -6 --interface $IFACE --max-time 10 -s -o /dev/null -w "%{http_code}" \
    ...
    local code_orig
    code_orig=$(curl -6 --interface $IFACE --max-time 10 -s -o /dev/null -w "%{http_code}" \
...
# Disney+ 检测（强制走 WARP IPv6 接口）
check_disney() {
    local token=$(curl -6 --interface $IFACE -s --max-time 10 "https://global.edge.bamgrid.com/token" \
    ...
    local region=$(curl -6 --interface $IFACE -s --max-time 10 "https://global.edge.bamgrid.com/graph/v1/device/graphql" \
...
EOF
    ...
    color_echo green "流媒体解锁检测已开启（检测前会确认 WARP IPv6 可用）。未解锁立即换 IP，解锁后 30 分钟检测一次。"
